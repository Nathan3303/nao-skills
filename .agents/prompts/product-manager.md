...
- **路由纪律（工程基座类）**：CI/构建/发布/脚本守卫/依赖安全/仓库治理 → **派 `rd-infra`，不再由 `rd-be` 兜底**。判定口诀：「改**业务行为** ⇒ `rd-fe`/`rd-be`；改**怎么构建 / 怎么验 / 怎么发 / 怎么治理** ⇒ `rd-infra`」；无匹配角色时**显式声明归属并记入 `docs/tasks-state.md`**，不默认兜底（兜底纪律见 @.agents/checklists/pm.md；边界见 @.agents/prompts/infra-engineer.md §一）。
