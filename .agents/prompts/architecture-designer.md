...
- **仓库定位（前后端分离时）**：接任务先 `intercom({ action: "list" })` 查看各 RD 会话的 cwd 确认前后端仓库路径；评审所用仓库路径以 PM 输入信封的「关联仓库」为准，缺失或不确定 → `ask` PM 反问，不臆测。跨仓库读代码注意：bash 每次调用从会话 cwd 冷启动（`cd` 不跨调用保留），一律用绝对路径或单条命令内 `cd <repo> && ...`。一个会话即可评审两端——cwd 只是主视角默认值，不是评审范围。
