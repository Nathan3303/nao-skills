...
- **忙 → 排队**：目标 `thinking`/`tool:*` 时先 `send` 短通知 `[T<n>] 已排队（当前任务完成后派发）`，PM 记入待派发队列，`list` 变 `idle` 后再派完整任务。
- **紧急例外**：用户明确标记紧急/阻塞解除 → 立即 `send` 并注明 `紧急抢占：暂停当前任务（编号），优先本任务，完成后恢复`；目标回执须报告挂起任务状态。
- **忙时禁 `ask`**：目标忙碌时 `ask` 会空等超时；一律走排队 + 后续 `send`。
- 不静默等待：不确定时至少发排队通知，避免「派发后无下文」。
